...
export function Categories({tabs,onTabChange}) {
 ...
 const handleTab = tab => {
  ...
  return (
    ...
    <Alert visible={alert_visible} onCancel={()=> {setAlert(false),setTab([])}} onProceed={()=> { setAlert(false),setTab([])}} title={`Delete ${tabData.name} category?`} content={`Are you sure you want to delete ${tabData.name} category?`} confirmTitle="Proceed"/>
        <ScrollView  
          horizontal={true}
