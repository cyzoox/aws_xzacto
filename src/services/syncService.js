      ...
      for (const change of storePendingChanges) {
        try {
          if (change.type === 'CREATE') {
            ...
            store.dispatch(storeSyncComplete({
      ...
      for (const change of staffPendingChanges) {
        try {
          if (change.type === 'CREATE') {
            ...
            const input = {
              ...change.data,
              role: Array.isArray(change.data.role) ? change.data.role : [change.data.role]
            };
            ...
            store.dispatch(staffSyncComplete({
      ...
      for (const change of state.sales.pendingChanges) {
        try {
          if (change.type === 'CREATE') {
            ...
            store.dispatch(salesSyncComplete({
