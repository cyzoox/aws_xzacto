...
export const isBluetoothDeviceConnected = () => {
  return BluetoothManager.isBluetoothEnabled()
    .then(() => BluetoothManager.getConnectedDeviceAddress());
};
...
export const printReceipt = async (
  ...
  try {
    ...
    await BluetoothEscposPrinter.printText(`Tel: ${storeInfo.phone || ''}\n`, {});
    ...
    await BluetoothEscposPrinter.printText(`Invoice #: ${transaction.id || ''}\n`, {});
    await BluetoothEscposPrinter.printText(`Cashier: ${transaction.cashierName || ''}\n`, {});
    ...
    // Subtotal
    const subtotal = cartItems.reduce((total, item) => total + (item.sprice * item.quantity), 0);
    await BluetoothEscposPrinter.printColumn(
      [24, 24],
    ...
    if (transaction.discount && transaction.discount > 0) {
      ...
      await BluetoothEscposPrinter.printColumn(
        [24, 24],
        [BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.RIGHT],
        [`DISCOUNT (${transaction.discount}%):`, `-₱${discountAmount.toFixed(2)}`],
        {},
      );
    }
...
export const printXReport = async (
  ...
  try {
    ...
    await BluetoothEscposPrinter.printText(`Report Time: ${new Date().toLocaleString()}\n`, {});
    await BluetoothEscposPrinter.printText(`Period: ${startDate.toLocaleString()} - ${endDate.toLocaleString()}\n`, {});
    await BluetoothEscposPrinter.printText(`Cashier: ${cashierName || 'All'}\n`, {});
    
    await BluetoothEscposPrinter.printText("--------------------------------\n", {});
    
    // Sales summary
    const totalSales = transactions.reduce((total, t) => total + (t.totalAmount || 0), 0);
    const totalItems = transactions.reduce((total, t) => {
      // Assume each transaction has an items array with quantities
      return total + (t.items?.reduce((sum, item) => sum + item.quantity, 0) || 0);
    }, 0);
    ...
    await BluetoothEscposPrinter.printText("Registers are still active\n\n", {});
...
export const printZReport = async (
  ...
  try {
    ...
    transactions.forEach(transaction => {
      if (transaction.items && transaction.items.length > 0) {
        transaction.items.forEach(item => {
          ...
          categories[item.category].amount += item.sprice * (item.quantity || 1);
        });
      }
    });
    ...
    await BluetoothEscposPrinter.printText("Register data has been reset\n\n", {});
