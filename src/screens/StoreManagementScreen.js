...
export default function StoreManagementScreen({navigation}) {
  ...
  const {items: stores, loading} = useSelector(state => {
    ...
    return {
      items: currentUserId
        ? allStores.filter(
            store => store.ownerId === currentUserId && !store._deleted,
          )
        : [],
  ...
  const handleAddStore = async () => {
    ...
    if (currentLimit > 0 && currentStoreCount >= currentLimit) {
      ...
      Alert.alert(
        ...
        } plan allows a maximum of ${currentLimit} stores. Please upgrade your subscription to add more stores.`,
        [
          {text: 'Cancel', style: 'cancel'},
          {
            text: 'Upgrade Subscription',
            onPress: () =>
              navigation.navigate('Subscription', {staffData: staff[0]}),
          },
        ],
      );
      return;
