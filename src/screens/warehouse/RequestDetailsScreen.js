...
const RequestDetailsScreen = ({navigation, route}) => {
  ...
  const fetchRequestDetails = async () => {
    ...
    try {
      ...
      console.log(
        'Retrieved inventory request:',
        JSON.stringify(inventoryRequest, null, 2),
      );
  ...
  const handleSubmitFulfill = async () => {
    ...
    try {
      ...
      const totalFulfilled = currentItem.fulfilledQuantity
        ? currentItem.fulfilledQuantity + newFulfilledQty
        : newFulfilledQty;
      ...
      if (quantity > 0 && currentItem.product) {
        ...
        if (request.storeId && quantity > 0) {
          try {
            console.log(
              `Transferring ${quantity} units of ${product.name} to store ${request.storeId}`,
            );
            ...
            console.error(
              'Error transferring product to store:',
              transferError,
            );
  ...
  const updateRequestStatus = async () => {
    try {
      ...
      const allFulfilled = items.every(
        item => item.status === ITEM_STATUS.FULFILLED,
      );
  ...
  const completeRequest = async () => {
    ...
    const incomplete = requestItems.filter(item => {
      return (
        item.status !== ITEM_STATUS.FULFILLED &&
  ...
  const getStatusColor = status => {
    switch (status) {
      ...
      case REQUEST_STATUS.PROCESSING:
        return '#2196F3'; // Blue
      case REQUEST_STATUS.PARTIALLY_FULFILLED:
        return '#8BC34A'; // Light Green
      case REQUEST_STATUS.FULFILLED:
        return '#4CAF50'; // Green
      case REQUEST_STATUS.CANCELLED:
        return '#F44336'; // Red
      case ITEM_STATUS.OUT_OF_STOCK:
        return '#F44336'; // Red
      default:
        return '#757575'; // Grey
    }
