...
const StoreScreen = ({navigation}) => {
  ...
  const {items: stores, loading: storeLoading} = useSelector(
    state => state.store,
  );
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      try {
        ...
        const userRole = Array.isArray(staffData.role)
          ? staffData.role[0]
          : staffData.role;
        ...
        const filteredStores = storeData.listStores.items.filter(
          s => !s._deleted,
        );
        console.log(
          `Directly fetched ${filteredStores.length} stores for user ${userId}`,
        );
  ...
  const refreshStores = async () => {
    try {
      ...
      console.log(
        `Refreshed ${filteredStores.length} stores for user ${userId}`,
      );
  ...
  const isDefaultStore = store => {
    ...
    if (defaultStoreId && store.id === defaultStoreId) {
      return true;
    }
            ...
            <View style={styles.centered}>
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyText}>
                  You don't have any stores yet. Please contact your
