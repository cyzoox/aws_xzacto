...
import { Text, View, ScrollView, TouchableOpacity, StyleSheet, Alert, ActivityIndicator, Switch, KeyboardAvoidingView, Platform } from "react-native";
import { TextInput, Card, Title, Divider, Button, Checkbox, HelperText } from "react-native-paper";
...
const StoreSettings = ({ route, navigation }) => {
  ...
  const loadLocalSettings = async () => {
    try {
      ...
      const generalSettingsStr = await AsyncStorage.getItem(`store_general_${STORE.id}`);
      ...
      const operationalSettingsStr = await AsyncStorage.getItem(`store_operational_${STORE.id}`);
      if (operationalSettingsStr) {
        const operationalSettings = JSON.parse(operationalSettingsStr);
        setVat(operationalSettings.vat ? operationalSettings.vat.toString() : "0");
        setLowStock(operationalSettings.lowStock ? operationalSettings.lowStock.toString() : "0");
  ...
  const renderTabs = () => (
    <View style={styles.tabsContainer}>
      ...
      >
        <MaterialIcons name="settings" size={20} color={activeSection === 'general' ? colors.primary : '#777'} />
        <Text style={[styles.tabText, activeSection === 'general' && styles.activeTabText]}>General</Text>
      </TouchableOpacity>
      
      <TouchableOpacity 
        style={[styles.tab, activeSection === 'operational' && styles.activeTab]}
        onPress={() => setActiveSection('operational')}
      >
        <MaterialIcons name="business" size={20} color={activeSection === 'operational' ? colors.primary : '#777'} />
        <Text style={[styles.tabText, activeSection === 'operational' && styles.activeTabText]}>Operational</Text>
      </TouchableOpacity>
      ...
      >
        <MaterialIcons name="security" size={20} color={activeSection === 'security' ? colors.primary : '#777'} />
        <Text style={[styles.tabText, activeSection === 'security' && styles.activeTabText]}>Security</Text>
      </TouchableOpacity>
    </View>
  );
  ...
  const renderGeneralSettings = () => (
    <Card style={styles.card}>
      <Card.Content>
        ...
        />
        {errors.storeName && <HelperText type="error">{errors.storeName}</HelperText>}
  ...
  const renderOperationalSettings = () => (
    <Card style={styles.card}>
      <Card.Content>
        ...
        />
        {errors.lowStock && <HelperText type="error">{errors.lowStock}</HelperText>}
        
        <View style={styles.settingRow}>
          <View style={styles.settingTextContainer}>
            <Text style={styles.settingLabel}>Cashier Sales View</Text>
            <Text style={styles.settingDescription}>Allow cashiers to view sales data and reports</Text>
          </View>
          <Switch
            value={cashierView}
        ...
        <View style={styles.settingRow}>
          <View style={styles.settingTextContainer}>
            ...
            <Text style={styles.settingDescription}>Allow sales on credit to customers</Text>
          </View>
          <Switch
            value={allowCredit}
      ...
      <Card.Content>
        ...
        <View style={styles.settingRow}>
          <View style={styles.settingTextContainer}>
            ...
            <Text style={styles.settingDescription}>Staff must login before accessing the system</Text>
          </View>
          <Switch
        ...
        <TextInput
          ...
          mode="outlined"
          style={styles.input}
          right={<TextInput.Icon name={rightIcon} onPress={handlePasswordVisibility} />}
